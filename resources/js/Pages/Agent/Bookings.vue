    ...
    <AgentLayout>
        <div class="space-y-5">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 class="text-lg font-semibold text-gray-800">My Bookings History</h1>
                    <p class="text-xs text-gray-400 mt-0.5">Track all patient doctor appointments and medical tests booked through your agent account</p>
                </div>
            ...
            <div v-if="currentTab === 'appointments'" class="bg-white rounded-lg shadow-sm overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs text-gray-600">
                        <thead class="bg-gray-50 text-xs font-semibold text-gray-600 border-b border-gray-200">
                            <tr>
                                <th class="px-5 py-3">Patient Details</th>
                                <th class="px-5 py-3">Doctor & Specialization</th>
                                <th class="px-5 py-3">Date & Serial</th>
                                <th class="px-5 py-3">Visit Fee</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="apt in appointments.data" :key="apt.id" class="hover:bg-gray-50">
                                <td class="px-5 py-3.5">
                                    <div class="font-medium text-gray-900">{{ apt.name }}</div>
                                    <div class="text-2xs text-gray-400 font-mono">{{ apt.phone }}</div>
                                </td>
                                <td class="px-5 py-3.5">
                                    <div class="font-medium text-gray-800">{{ localized(apt.doctor?.name) || apt.preferred_doctor || 'Doctor' }}</div>
                                    <div class="text-2xs text-blue-600">{{ localized(apt.doctor?.specialization?.name) || apt.department }}</div>
                                </td>
                                <td class="px-5 py-3.5">
                                    <div class="font-medium text-gray-800">{{ apt.appointment_date }}</div>
                                    <div class="text-2xs text-purple-600 font-medium">Serial #{{ apt.serial_number || 'TBD' }} • {{ apt.time_slot }}</div>
                                </td>
                                <td class="px-5 py-3.5 font-mono font-semibold text-gray-900">
                                    BDT {{ Number(apt.fee || 0).toLocaleString() }}
                                </td>
                                <td class="px-5 py-3.5">
                                    <div class="space-y-1">
                                        <span :class="paymentBadge(apt.payment_status)" class="px-2 py-0.5 rounded-full text-3xs font-bold uppercase">
                                            {{ apt.payment_status }}
                                        </span>
                                        <div v-if="apt.payment_method" class="text-3xs text-gray-400 uppercase font-mono">
                                            {{ apt.payment_method }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-5 py-3.5">
                                    <span :class="statusBadge(apt.status)" class="px-2 py-0.5 rounded-full text-2xs font-semibold uppercase">
                                        {{ apt.status }}
                                    </span>
                                </td>
                                <td class="px-5 py-3.5">
                ...
                <!-- Pagination -->
                <div v-if="appointments.links?.length > 3" class="p-3.5 border-t border-gray-100 flex justify-center gap-1">
                    <Link v-for="(link, i) in appointments.links" :key="i" :href="link.url || '#'" :class="[link.active ? 'bg-blue-600 text-white font-semibold' : 'text-gray-600 hover:bg-gray-100', !link.url ? 'opacity-50 cursor-not-allowed' : '']" class="px-3 py-1 rounded text-xs" v-html="link.label">
                    </Link>
                </div>
            </div>
