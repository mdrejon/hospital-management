    ...
    <AdminLayout>
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Edit Agent: {{ agent.user?.name }}</h1>
                    <p class="text-xs text-gray-500 mt-1">Code: {{ agent.agent_code }} &bull; Current Balance: BDT {{ Number(agent.wallet_balance).toLocaleString() }}</p>
                </div>
                <div class="flex items-center gap-3">
                    <Link :href="route('admin.agents.show', agent.id)" class="px-3.5 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100">
                        View Details
                    </Link>
                    <Link :href="route('admin.agents.index')" class="px-3.5 py-1.5 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                        &larr; Back
                    </Link>
                </div>
            </div>
                ...
                <div class="bg-white rounded-xl border border-gray-200/80 shadow-xs p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input v-model="form.name" type="text" required class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                        ...
                        <div>
                            ...
                            <input v-model="form.email" type="email" required class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                        ...
                        <div>
                            ...
                            <input v-model="form.phone" type="text" required class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                            <span v-if="form.errors.phone" class="text-xs text-red-600">{{ form.errors.phone }}</span>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Change Password (leave blank to keep)</label>
                            <input v-model="form.password" type="password" class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="New password" />
                        ...
                        <div>
                            ...
                            <input v-model="form.nid_number" type="text" class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Replace NID Document</label>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input v-model="form.address" type="text" class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border border-gray-200/80 shadow-xs p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div v-if="form.payout_method !== 'bank'">
                            ...
                            <input v-model="form.payout_account_number" type="text" class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                        </div>
                    ...
                    <div v-if="form.payout_method === 'bank'" class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <div>
                            ...
                            <input v-model="form.bank_name" type="text" class="w-full px-3 py-2 text-sm border rounded-lg bg-white focus:outline-none" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Branch Name</label>
                            <input v-model="form.bank_branch" type="text" class="w-full px-3 py-2 text-sm border rounded-lg bg-white focus:outline-none" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Routing Number</label>
                            <input v-model="form.bank_routing" type="text" class="w-full px-3 py-2 text-sm border rounded-lg bg-white focus:outline-none" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Account Name & Number</label>
                            <input v-model="form.bank_account_name" type="text" class="w-full px-3 py-2 text-sm border rounded-lg bg-white focus:outline-none" />
                        </div>
                    </div>
                </div>
...
const bank = props.agent.bank_details || {};
...
const form = useForm({
    ...
    commission_type: props.agent.commission_type || 'percentage',
    doctor_commission_rate: props.agent.doctor_commission_rate || 10,
    test_commission_rate: props.agent.test_commission_rate || 15,
...
function submit() {
    form.post(route('admin.agents.update', props.agent.id), {
        forceFormData: true,
    });
}
</script>
