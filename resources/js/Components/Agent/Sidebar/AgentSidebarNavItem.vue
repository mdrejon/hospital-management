            ...
            <div v-if="!collapsed && item.children && open" class="bg-gray-50 border-l-2 border-blue-500 ml-4">
                <Link
                    ...
                    :class="[
                        ...
                        isAgentNavRouteActive(child.route)
...
import { isAgentNavRouteActive } from './agentActiveRoute';
...
const isActive = computed(() => {
    ...
    if (isAgentNavRouteActive(props.item.route)) return true;
    if (props.item.children) {
        return props.item.children.some(c => isAgentNavRouteActive(c.route));
    }
    return false;
});
